...
const drawGrid = (
  ...
  const priceRange = scale.max - scale.min;
  const stepSize = priceRange / 8;  // Divide into 8 sections
  
  ctx.strokeStyle = 'rgba(255, 255, 255, 0.07)'; // More visible grid lines
  ctx.lineWidth = 1;
  
  // Draw horizontal grid lines - stop before the price scale
  for (let price = scale.min; price <= scale.max; price += stepSize) {
    const y = priceToY(price, scale, chartAreaHeight);
    if (y < 0 || y > chartAreaHeight) continue; // Skip if outside of chart area
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.lineTo(chartAreaWidth, y); // Stop at the chart area width
    ctx.stroke();
  }
  
  // Draw vertical grid lines (time) - also stop before price scale and above time scale
  const verticalLines = 12; // Number of vertical grid lines
  const xStep = chartAreaWidth / verticalLines;
  
  for (let x = 0; x <= chartAreaWidth; x += xStep) {
    ctx.beginPath();
    ctx.moveTo(x, 0);
...
const formatTimeLabel = (timestamp: number, timeframe: string): string => {
  ...
  switch (timeframe) {
    case '1d':
      // For daily timeframe, show month names
      return date.toLocaleString('default', { month: 'short', year: '2-digit' });
    case '4h':
      // For 4h timeframe, show month with dates
      return date.toLocaleString('default', { 
        month: 'short', 
        day: 'numeric' 
      });
    case '1h':
      // For hourly timeframe, show days
      return date.toLocaleString('default', { day: 'numeric', month: 'short' });
    case '5m':
      // For 5min timeframe, show hours (every 3 hours)
      const hours = date.getHours();
      const hoursFormatted = hours.toString().padStart(2, '0');
      return `${hoursFormatted}:00`;
    case '1m':
      // For 1min timeframe, show 30 min intervals
...
const drawTimeScale = (
  ...
  // Display current timeframe and zoom level in the time scale
  let timeframeText = '';
  switch (timeframe) {
    case '1d': timeframeText = 'Daily'; break;
    case '4h': timeframeText = '4 Hour'; break;
    case '1h': timeframeText = '1 Hour'; break;
    case '5m': timeframeText = '5 Min'; break;
    case '1m': timeframeText = '1 Min'; break;
  ...
  if (effectiveBarWidth > barWidth) {
    ...
    timeframeText += ` (${zoomLevel}x)`;
  }
  
  ctx.fillStyle = '#888';
  ctx.fillText(timeframeText, chartAreaWidth - 10, chartAreaHeight + timeScaleHeight/2);
};
...
const drawPriceScale = (
  ...
  for (let i = 0; i < numLabels; i++) {
    ...
    // Draw line (more subtle if minimizeGrid is true)
